...
iPhone can keep the AACP/HFP session so crown volume hits the phone. **Soft exclusive (default on)** HCI-drops extra Pico ACLs while USB wants the sink and keeps the iPhone paired. Firmware still claims **`0x06` own** and LibrePods **`0x20=0x01`** on every AACP session. When **`softexcl off`**, it **re-sends `0x06`/`0x20` plus LibrePods `0x10` takeOver** (media_info + showUI + hijack) when A2DP is stolen **and USB speaker is open**. Dual-connect may still need the user to set iPhone “Connect only When Last Connected”.
...
- **Soft exclusive vs fight path.** Default **on** (`softexcl on|off`, Quiet Dual-connect CDC toggle, `@STATUS softexcl=` / `sxphase=`). While USB wants the sink (speaker alt ≠ 0 **or** USB PCM streaming), HOLD: HCI-disconnect extra Pico ACLs that are **not** the AirPods Max / **not** self, and refuse new ones. iPhone stays **paired** (never Forget, never wipe keys, never turn Mac BT off, never HCI-drop the Max). USB idle (alt 0 **and** not streaming) → GRACE (2 s) → IDLE so the phone can reconnect. HOLD skips 0x11 / `owns=00` they-own give-up (would undo exclusive) and skips AVDTP reclaim / LibrePods `0x10` takeOver. **Fight path** (unexpected-pause reclaim + `0x10` hijack + 0x11 give-up) is the best-effort fallback when **`softexcl off`**. Handshake OWNS/`0x20` still sent. Topology: Pico’s ACL is to the Max; the iPhone↔Max dual-connect link cannot be HCI-dropped from the dongle — CDC logs `no Pico ACL to phone` in that case.
