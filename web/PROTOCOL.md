...
Cross-correlation lag between L and R is **not** measured (deferred — too much CPU on the Pico). Dual-connect keys on `@STATUS` (same firmware state as flags2 / 0x0E parse — no invented opcodes): `reclaim=` steal-reclaim armed, `paused=` `we_paused_after_giveup`, `spk=` USB speaker open, `stream=` USB PCM streaming, `peer=` last `0x0E` audio-src MAC (`-` until the first parse), **`softexcl=`** `1` on / `0` off (default on), **`sxphase=`** `idle|hold|grace`. Soft exclusive: while USB wants the sink (`spk` or `stream`), firmware HCI-disconnects extra Pico ACLs that are not the Max / not self and refuses new ones; iPhone stays **paired** (never Forget, never wipe keys). After USB idle (`spk=0` and `stream=0`) a 2 s GRACE, then the phone may reconnect. `softexcl off` restores the AVDTP reclaim / LibrePods `0x10` fight path. Pico cannot HCI-drop the iPhone↔Max link (that ACL is on the headset); CDC may log `no Pico ACL to phone`. After iPhone steal/reclaim, CDC may log `[A2DP] steal → HID Pause`, `[A2DP] reclaim stream → host session wake`, `[A2DP] host session wake → HID Play`, `[USB] speaker FU unmute interrupt`, `[USB] speaker iso nudge` — Windows should resume TinyUSB PCM without switching the default device. If `spk=1 stream=0` persists, the old recovery is still: switch default audio device away from TinyUSB and back.
...
`softexcl on|off` — soft exclusive (default **on**). While USB wants the sink, drop extra Pico HCI ACLs that are not the AirPods Max; keep the iPhone **paired**. USB idle + 2 s grace allows the phone back. Off restores reclaim / `0x10` fight. Quiet’s Dual-connect strip has a CDC toggle. HID flags2 is full — this is serial/`@STATUS` only.
...
```
@STATUS a2dp=1 aacp=1 mic=0 gain=6 slot=1 mute=0 … owns=1 duck=0 autocon=1 allowauto=0 earen=1 reclaim=0 paused=0 spk=1 stream=1 peer=aa:bb:cc:dd:ee:ff softexcl=1 sxphase=hold … last19=05 01 name=AirPods Max … spk_misalign=0 spk_rem=0 spk_half=0 spk_swap=0 spk_l=-12 spk_r=-13
@GAIN 6
