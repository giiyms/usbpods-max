...
| Ear detection report | `0x0006` | parsed + HID | `04 00 04 00 06 00 [primary] [secondary]`. **`0x00` = In Ear = ON HEAD**. `0x01` Out, `0x02` In Case. L=0 R=0 while worn is ON. **Off-head = either cup not 0x00** (Apple Max: pause if you lift one earphone; LibrePods linux default `PauseWhenOneRemoved`). HID Consumer **Pause** (`0xB1`) on take-off if A2DP streaming; **Play** (`0xB0`) on put-on only if we paused. Never Play/Pause toggle. ~200 ms debounce. First packet after connect: no HID. Max 2 is a headset. |
...
iPhone can keep the AACP/HFP session so crown volume hits the phone. Firmware always claims **`0x06` own** and LibrePods **`0x20=0x01`**. Dual-connect may still need the user to set iPhone “Connect only When Last Connected”.
...
- On `AVDTP_SUBEVENT_SIGNALING_CONNECTION_RELEASED`, wait 400 ms then retry every 2 s, max 30, unless `reclaim_hold` (explicit disconnect / pair-scan).
- USB speaker alt ≠ 0 and A2DP down → `control_request_reconnect()`.
